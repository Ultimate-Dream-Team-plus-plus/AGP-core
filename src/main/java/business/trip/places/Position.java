...
public class Position {
	...
	/*
	 * This method implements the Haversine formula to calculate the distance between two points
	...
	public double computeDistance(Position arrival) {
		...
		double a = Math.pow(Math.sin((arrival_lat - departure_lat)/2), 2) + Math.cos(departure_lat) * Math.cos(arrival_lat) * Math.pow(Math.sin((arrival_long - departure_long) / 2), 2);
